...
class BalancesCalculator:
    ...
    def __init__(self, args: Args):
        self._account_balance: Decimal = Decimal(str(args.fzrox_amount)) \
                                       + Decimal(str(args.fxnax_amount))
        self._fzrox_balance: Decimal = Decimal(str(args.fzrox_amount))
        self._fxnax_balance: Decimal = Decimal(str(args.fxnax_amount))
        self._deposit_amount: Decimal = Decimal(str(args.deposit_amount))
        self._balance_after_deposit = self._account_balance + \
                                      self._deposit_amount
        self.deposit_insufficient_to_balance = False
        self._fzrox_percentage: Decimal = Decimal(str(STOCK_PERCENTAGE / 100))
        self._fxnax_percentage: Decimal = 1 - self._fzrox_percentage
        ...
        self._to_fzrox: Decimal = self.get_fzrox_amount()
        self._to_fxnax: Decimal = self.get_fxnax_amount()

    def get_fzrox_amount(self) -> Decimal:
        diff: Decimal = self._target_fzrox_amount - self._fzrox_balance
        if self._deposit_amount < diff:
            self.deposit_insufficient_to_balance = True
        return diff

    def get_fxnax_amount(self) -> Decimal:
        diff: Decimal = self._target_fxnax_amount - self._fxnax_balance
        if self._deposit_amount < diff:
            self.deposit_insufficient_to_balance = True
        return diff
